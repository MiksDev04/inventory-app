...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $stmt = $pdo->prepare("UPDATE suppliers SET name = ?, contact_person = ?, contact_email = ?, contact_phone = ?, updated_at = NOW() WHERE id = ?");
        ...
        header("Location: index.php?success=Supplier updated successfully");
        exit;
    } catch (Exception $e) {
...
// Use supplier data as default form data if no error occurred
if (!isset($formData)) {
    $formData = $supplier;
}
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white border-0 pb-0">
                <h5 class="card-title mb-0">
                    <i class="bi bi-pencil-square text-primary me-2"></i>Supplier Details
                </h5>
            </div>
            <div class="card-body">
